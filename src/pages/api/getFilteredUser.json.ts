...
export async function GET({ url }) {
  ...
  if (!day || !turn) {
    return new Response(JSON.stringify({ error: 'Day and turn parameters are required' }), {
      status: 400,
  ...
  try {
    ...
    console.error(error);
    return new Response(JSON.stringify({ error: 'Failed to fetch users by availability' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
